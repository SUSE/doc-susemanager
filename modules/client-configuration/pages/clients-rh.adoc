...
{redhat} clients require a {redhat} certificate authority (CA) and entitlement certificate and entitlemente key.
...
Depending on where the {redhat} system is registered, the URL’s to which the system is authorized to replicate differ.

{redhat} systems can be registered to three different infrastructures:

1. {redhat} CDN (redhat.com)
2. {redhat} Satellite Server
3. RHUI (update infrastructure in the cloud)

This guide focuses on a system registered to {redhat} CDN (redhat.com). Register at least one system there with an authorized subscription to obtain repository content.

[IMPORTANT]
====
Entitlement certificates for RHUI (cloud-based systems) only authorize you to get content there.
Satellite certificates for client systems require a Satellite server and subscription: such client certificates are UNSUPPORTED with {productname} server.
====
