...
== Register {rhel} Salt Clients


Before you register {rhel} Salt clients to your {productname} Server, check that you have the corresponding {rhel} product enabled, and the required channels are fully synchronized.
...
[IMPORTANT]
====
Missing packages will cause your registtration to fail.
...
When you are ready to register your {rhel} Salt client, follow the instructions in xref:client-configuration:registration-overview.adoc[].
