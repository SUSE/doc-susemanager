...
On {slsa}{nbsp}11 systems, salt clients can sometimes have SSL errors which make some operations unusable, including package management and bootstrapping.
...
To fix this error,open the [path]``/etc/apache2/ssl-global.conf`` configuration file, locate the [systemitem]``SSLProtocol`` line, and update it to read:
