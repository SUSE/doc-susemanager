...
zypper in spacewalk-utils
----
. Add the {centos} base, updates, and client channels using the [command]``centos6-uyuni-client`` script:
+
----
centos6-uyuni-client -u admin -p `secret` -a x86_64 'centos7'
centos6-uyuni-client -u admin -p `secret` -a x86_64 'centos7-updates'
----
