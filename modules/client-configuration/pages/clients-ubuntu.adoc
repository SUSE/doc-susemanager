...
This passwordless root access is only needed for onboarding the client. Once the salt-minion is installed, this is no longer needed, because salt-minion is running with root privileges. So it is recommended to remove this line after the minion has been successfully onboarded.
