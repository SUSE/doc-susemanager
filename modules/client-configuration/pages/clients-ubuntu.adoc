...
This section describes how to synchronize the repositories and prepare {ubuntu} clients for registration to your {susemgr} Server.


== {ubuntu} Channel Management Tips

This section provides tips on {ubuntu} channel management.

The {ubuntu} base channel does not contain any packages, and {suse} does not provide any base packages.
The {ubuntu} media should be added as child channels of the {ubuntu} parent channel.
The {ubuntu} and tools channels are provided by SUSE Customer Center (SCC) using [command]``mgr-sync``.


== Synchronizing {ubuntu} Repositories

This section guides you through setting up {ubuntu} repositories as a {productname} channel.
All packages on the selected {ubuntu} repositories will be mirrored into a child channel located under the corresponding {ubuntu} distribution.


.Procedure: Synchronizing {ubuntu} Repositories

. Make sure to add the desired {ubuntu} distribution by using [command]``mgr-sync``.
...
Ensure that in the [guimenu]``Architecture`` field, select [systemitem]``AMD64 Debian``.
