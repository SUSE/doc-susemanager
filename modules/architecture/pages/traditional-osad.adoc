...
* The [systemitem]``mgr-osad`` service runs on the client. It responds to pings from [systemitem]``osa-dispatcher`` and runs [command]``mgr_check`` to execute actions when directed to do so.
...
If it finds a client that has not shown activity recently, it will use [systemitem]``jabberd`` to ping all [systemitem]``mgr-osad`` instances running on all clients registered with your {productname} server.
The [systemitem]``mgr-osad`` instances respond to the ping using [systemitem]``jabberd``, which is running in the background on the server.
...
This section covers enabling the [systemitem]``osa-dispatcher`` and [systemitem]``mgr-osad`` services, and performing initial setup.
...
SSL is required for [systemitem]``mgr-osad`` communication.
...
. On the client systems, as the root user, start the [systemitem]``mgr-osad`` service:
+

----
systemctl start mgr-osad
----
+
Because [systemitem]``mgr-osad`` and [systemitem]``osa-dispatcher`` are run as services, you can use standard commands to manage them, including [command]``stop``, [command]``restart``, and [command]``status``.
...
| [systemitem]``mgr-osad``             | Client   | [path]``/etc/sysconfig/rhn/mgr-osad.conf`` [path]``/etc/syseconfig/rhn/up2date``
| [systemitem]``mgr-osad``  log file   | Client   | [path]``/var/log/mgr-osad``
