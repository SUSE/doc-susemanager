...
{productname} Proxy requires additional configuration to make it useful.
...
// REMARK 2019-08-23, ke: Is this a proxy-only feature?
[NOTE]
.Proxy Chains
====
...
Make sure the proxie's TCP ports `4505` and `4506` are open.
...
.Country Code
====
As the country code enter the country code set during the {productname} installation.
...
Also run again the script if a HTTP error was met during script execution.
...
[[proxy.pxe.setup]]
== Enable PXE Boot through {susemgrproxy}
...
To enable PXE boot through a proxy server, additional software must be installed and configured on both the {productname} Server and the {susemgrproxy} Server.
...
Consider copying important data to a central place that can also be accessed by the new proxy server.
