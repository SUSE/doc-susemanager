...
// here it is unclear that the script needs to be run on the proxy (it gets clearer only by the next topic which says you also need to configure the server
