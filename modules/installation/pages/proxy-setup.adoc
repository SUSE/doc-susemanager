...
cd /root/ssl-build
scp root@PARENT:/usr/share/rhn/RHN-ORG-PRIVATE-SSL-KEY .
scp root@PARENT:/usr/share/rhn/RHN-ORG-TRUSTED-SSL-CERT .
scp root@PARENT:/usr/share/rhn/rhn-ca-openssl.cnf .
----
...
. Shutdown the server.
. Install a new {productname}{nbsp}{productnumber} Proxy, see pass:c[xref:FILENAME.adoc#at.manager.proxy.inst-and-clients[]].
