...
. OPTIONAL: In the [guimenu]``Add On Product`` screen, select any additional or add-on products you require, and click btn:[Next]. Note that it is not recommended to run any other workloads on your {productname}. So only add add-ons that you absolutely require, like driver repositories from your hardware vendor.
