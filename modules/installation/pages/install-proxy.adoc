...
[[installation-proxy-prep]]
== Prepare SLES for SUSE Manager Proxy

[[proc.jeos.susemgr.prep]]
.Procedure: Preparing SLES for {productname} Proxy Installation
...
. Register {productname} Proxy with SCC (for example, replace `<productnumber>` with `{productnumber}` and `<architecture>` with `x86_64`):
+

----
SUSEConnect -e<EMAIL_ADDRESS> -r<SUSE_MANAGER_PROXY_CODE> \
  -p sle-module-suse-manager-proxy/<productnumber>/<architecture>
----
