...
You can run {productname} Server on a cloud instance from a third-party provider such as Amazon EC2, or Microsoft Azure.
...
We recommend that the repositories and the database for {productname} are stored on a virtual storage device.
...
Provision a disk device in the public cloud environment, according the cloud service provider's documentation.
