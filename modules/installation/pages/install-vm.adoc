...
[NOTE]
====
In the following table minimal requirements are specified, which are suitable for a quick test installation (server with one client).
If you want to use it as a production environment, consider the requirements listed in xref:hardware-requirements.adoc[].
====
