...
This section covers {productname} Server setup.
...
{productname} is a complex system, and therefore installing third party is not allowed.
...
. When the setup has completed, click btn:[Next] to continue.
...
* Start product synchronization by clicking btn:[Add Products].
...
After the channel synchronization process has completed continue with xref:client-configuration:manual-registration-overview.adoc[Client Registration].
