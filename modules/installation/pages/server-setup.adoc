...
[WARNING]
.Installation of software on {productname} hosts
--
{productname} is part of the {sle} 4.0 product family and thus compatible with the software shipped with {sles}.

{productname} is a complex system, and therefore installing software is not allowed.
...
https://packagehub.suse.com/[SUSE PackageHub] provides community Packages for {sles}. PackageHub may bring newer packages to the {productname} Server and {productname} Proxy, potentially causing unrecoverable collateral damages and errors which will require reinstallation of {productname}. Do not enable the PackageHub repositories on the {productname} hosts.
--
