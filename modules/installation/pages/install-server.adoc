...
For information on registering with {scc}, or obtaining installation media, see xref:[installation-general-requirements]
// I can't remember how we're supposed to do xrefs. Need to check with @jcayouette --LKB

[NOTE]
====
If you are installing {susemgr} as a proof of concept, or this is a test installation, you can follow these instructions on a virtual machine rather than in production.
Ensure your virtual machine has enough disk space and RAM by checking the requirements at xref:[install-hardware-requirements]
====
...
. Review the settings on the [guimenu]``Installation Settings`` screen, and once you are happy click btn:[Install].

Once the installation procedure has finished, you can check that you have all the required modules by using the [command]``SUSEConnect --list-extensions`` command at a command prompt.
