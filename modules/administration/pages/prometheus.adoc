...
Medtrics are available for these services:
...
Currently, two exporters are supported:
  ...
  - golang-github-prometheus-node_exporter - Node exporter
  - golang-github-wrouesnel-postgres_exporter - PostgreSQL exporter

Installing and configuring exporters can be done using a Salt formula.
...
#### Configuration
  ...
  - TODO: describe how to enable the formula

##### Node exporter

  - Node exporter can be enabled or disabled. Currently there are no configuration options.
...
## Grafana
  ...
  - Grafana supports multiple backends, not only Prometheus
...
### Visualizing SUSE Manager metrics

  - TODO: Sample SUSE Manager dashboard
////
