...
[NOTE]
====
Depending on your environment and settings, you might receive an ``invalid certificate`` error when you connect to the server.
If this occurs, check that your SSL certificate is current, run the [command]``update-ca-certificates`` command on the command line to re-trust your certificate, and restart the spacewalk services.
====
