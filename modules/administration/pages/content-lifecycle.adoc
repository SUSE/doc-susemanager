...
{productname} allows you to create various types of filters to control the content on project build. Below is the list of supported filters:
...
Moreover, each filter has ``rule`` parameter set to either ``Allow`` or ``Deny`` value. The filters are processed as follows:
- if a package or patch satisfies a ``Deny`` filter, it will be excluded from the result,
- if a package or patch satisfies an ``Allow`` filter, it will be included in the result (even if it was excluded a ``Deny`` filter).
