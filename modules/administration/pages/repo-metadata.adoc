...
You can list your available keys in short format:
+
----
...
----
. Restart the metadata services and schedule metadata regeneration to pick up the changes:
+
----
mgr-sign-metadata-ctl regen-metadata
----
. OPTIONAL: If you want to regenerate channel metadata with signing enabled, use this command:
+
----
mgr-sign-metadata-ctl regen-metadata
----
...
Once it is enabled, it is enabled on all software channels on the server.
...
. On RPM-based client systems, use these remote commands:
+
----
...
. OPTIONAL: For salt minions, you might prefer to use  a state to manage your GPG keys.
