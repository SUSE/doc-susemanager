...
Prometheus collects metrics using HTTP pulls, allowing for higher performance and scalability.
Prometheus is an open-source software project, written in Go.
Its source code is available at https://github.com/prometheus/.
...
When you have the exporters installed and configured, you can manually configure monitoring, or set up service discovery so that Prometheus automatically starts scraping metrics from systems as they are enabled.
...
. Open the Prometheus static configuration file at [path]``/etc/prometheus/prometheus.yml``.
