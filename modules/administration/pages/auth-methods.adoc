...
To configure the system wide authentication you can use YaST.
You will need to install the [package]``yast2-ldap-client`` and [package]``yast2-kerberos-client`` packages.
For more information about configuring PAM, see the SUSE Linux Enterprise Server Security Guide https://documentation.suse.com/sles/15-SP1/html/SLES-all/part-auth.html.
This is a generic example that will also work for other network-based authentication methods.
It describes also how to configure the use of an Active Directory Service.
