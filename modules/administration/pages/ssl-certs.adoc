...
== Replace Existing CA and Server Certificates with third party certificiate
...
----
+
This command might end with a warning that the server certificate request file could not be found.
This file is not required, but you can provide it, by copying it into the server directory as `server.csr`.
+
----
cp <Certificate_Request_File>.csr /root/ssl-build/<Server_Name>/server.csr
----
+
When you repeat the command to generate the server certificate RPM, the warning should not be shown anymore.

Now you have an ssl-build directory similar to the one generated by "Procedure: Creating New Certificates" from the chapter before.
You can now continue with the "Procedure: Create Combined Certificate RPMs" from the "Create and Replace CA and Server Certificates" chapter.
