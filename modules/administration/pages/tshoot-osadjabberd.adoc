...
If you receive an ``open file count exceeded`` error, it could be because the maximum number of files that jabber can open is lower than the number of connected OSAD clients.
...
You can increase the number of files available to jabber by editing these lines [path]``/etc/security/limits.conf``:
...
The exact details will vary depending on your environment.
