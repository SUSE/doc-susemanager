...
When you rebuild the project with the btn:[Build] button, the new filter is used together with the the [guimenu]``by date`` filter we added before.
...
[IMPORTANT]
====
The [guimenu]``Allow`` filters rule overrides the exclude function of the [guimenu]``Deny`` filter rule (see xref:administration:content-lifecycle.adoc[Filter ``rule`` Parameter]).
====
