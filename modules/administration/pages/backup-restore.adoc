...
[IMPORTANT]
====
You will also need to back up your database, which you can do with the [command]``smdba`` tool.  The [command]``smdba`` tool is explained in <<bp.sect.backup.smdba.admin>>.
====

.Procedure: Restore from a Manual Backup
. Re-install {productname}.
For more information, see <<config-smdb.recover-root>>.
...
. Check whether you must restore [path]``/var/spacewalk/packages/``.
It can be neccessary if you did not have in your backup.
If the source repository still available, you can restore [path]``/var/spacewalk/packages/`` with a complete channel synchronization:
+
----
...
. Start by installing {productname}.
Do not mount the [path]``/var/spacewalk`` and [path]``/var/lib/pgsql`` partitions.
. When the installation of {productname} has completed shutdown the services with [command]``spacewalk-service shutdown`` and the database with [command]``rcpostgresql stop``.
. Mount your [path]``/var/spacewalk`` and [path]``/var/lib/pgsql`` partitions and restore the directories listed in <<backup-restore-product>>.
. Start {productname} services and the database with [command]``spacewalk-services start`` and [command]``rcpostgresql start``.
