...
. Stop the [command]``rhnsd`` daemon with [command]``/etc/init.d/rhnsd stop`` or on newer systemd-based systems, with [command]``systemctl stop rhnsd.service``. Then [command]``rcosad stop``.
