...
If your existing custom certificates have expired or stopped working for any reason, you can generate a new certificate from the existing CA.
...
Ensure that the [systemitem]``set-cname`` parameter is the fully-qualified domain name of your {productname} Server.
You can use the the [systemitem]``set-cname`` parameter multiple times if you require multiple aliases.
. Generate the CA certificate RPM:
+
----
rhn-ssl-tool --gen-ca --rpm-only --dir="/root/ssl-build" \
--from-ca-cert=`path_to_certificate_file`
----
. Generate the server certificate RPM:
+
----
rhn-ssl-tool --gen-server --rpm-only --dir="/root/ssl-build" \
--from-server-key=`path_to_server_key_file` \
--from-server-cert=`path_to_server certificate_file`
----
