...
Once the registration process is finished and all packages have been installed, enable the [daemon]``mgr-osad`` (Open Source Architecture Daemon).
...
----
systemctl enable mgr-osad
systemctl start mgr-osad
----

.[daemon]``mgr-osad``Together with [daemon]``rhnsd``
...
Adding [daemon]``mgr-osad`` ensures that.
The [daemon]``mgr-osad`` daemon receives commands over the jabber protocol from {productname} and commands are instantly executed.
