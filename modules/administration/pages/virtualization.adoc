...
.Using [daemon]``osad`` with [daemon]``rhnsd``
...
However, when managing {vmguest}s, you usually want actions like rebooting a {vmguest} to be carried out immediately, which can be done by adding [daemon]``osad``.
...
If you are using the [daemon]``rhnsd``, you may alternatively immediately trigger any scheduled actions by calling [command]``mgr_check`` on the {vmhost}.
