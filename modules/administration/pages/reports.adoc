...
| Activation Keys: Server Groups | [command]``activation-keys-groups`` | All activation keys and the server groups associated with each key.
...
| Custom Channels | [command]``custom-channels`` | Channel-meta-data for all channels owned by specific organizations
