...
Refer to xref:reference:configuration/config-channels.adoc[Configuration Channels] for channel creation instructions.

To manage the configuration of a system, install the latest [path]``mgr-cfg*`` packages.
Refer to xref:reference:configuration/config-menu.adoc#ref.webui.config.preparing[Preparing Systems for Configuration Management] for instructions on enabling and disabling scheduled actions for a system.
...
For more information on Cobbler and {kickstart} templates, refer to xref:client-configuration:cobbler.adoc[Cobbler]
...
Refer to xref:reference:systems/system-details/sd-remote-commands.adoc[Remote Command] for instructions.
...
For more details on OpenSCAP and audits, refer to xref:reference:audit/audit-menu.adoc[Audit Menu]
...
To cancel a reboot action, see xref:reference:schedule/pending-actions.adoc[Pending Actions]
