...
Refer to xref:reference:configuration/config-channels.adoc[] for channel creation instructions.
...
For more information on Cobbler and {kickstart} templates, refer to xref:client-configuration:cobbler.adoc[]
...
Refer to xref:reference:systems/system-details/sd-remote-command.adoc[] for instructions.
...
For more details on OpenSCAP and audits, refer to xref:reference:audit/audit-menu.adoc[]
...
To cancel a reboot action, see xref:reference:schedule/pending-actions.adoc[]
