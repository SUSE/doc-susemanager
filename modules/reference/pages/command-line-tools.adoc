...
[[spacewalk-repo-sync]]
== Synchronize Software Repositories into {productname} Channels (spacewalk-repo-sync)

[command]``spacewalk-repo-sync`` is an important tool on {productname}, which works most of the time in the background.
With [command]``spacewalk-repo-sync`` you mainly synchronize yum repositories into {productname} channels.

But for debugging or solving smaller issues it is useful to run it directly with special parameters.
...
Use the [option]``--deep-verify`` option to ignore cached package checksums, and thus solving checksum problems.
...
To find the root cause of synchronization problems it often helps to view the HTTP log when command]``spacewalk-repo-sync`` is running.
...
When debugging is done, disable the debug mode:
...
For a complete list of command line option, see the [command]``spacewalk-repo-sync`` manpage ([command]``man spacewalk-repo-sync``).
