...
----
+
NOTE: If the {productname} daemon ([command]``rhnsd``) or [command]``mgr-osad`` are running on the client system, the action will be picked up by these services.
...
----
+
If the {productname} daemon ([command]``rhnsd``) or [command]``mgr-osad`` are running on the client system, the action will be picked up by these services.
To check if they are running, use:
+
