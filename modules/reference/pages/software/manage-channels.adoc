...
====
Comps file might be associated with a repository and contains the packages group(s) available through the repository.

When syncing multiple repositories within the same channel, the last synced repository that contains a comps file will be used as the channel comps file.

If you still want to access all comps files, be sure to map a repository to a channel in a one-to-one fashion (1:1).
====