...
xrefs::
Some cross references are broken leading between books. Fixes for these will be submitted in an update.

search::
The search field has been removed from the {productname} documentation WebUI. Use the **Help -> Search** page in the WebUI to look for topics. Search will be integrated into the documentation WebUI in an update. Search is also available on link:https:/www.suse.com/documentation/suse-manager/[].
+
[NOTE]
====
From link:https:/www.suse.com/documentation/suse-manager/[] browse to {productname} 4.0 Documentation and use the search field to search in real-time through the docs.
====
