...
If you are using an older version, such as version 9.6, you must migrate PostgreSQL to version 10 before you begin the SUSE Manager migration.

// I would not talk about postgresql 9.4 at all. It is so out of date, nobody should (and in fact can) still use it. The reference to "Prepare to Upgrade" looks superfluous to me; it follows immediately anyway.
