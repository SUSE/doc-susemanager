...
For creating an autoinstallable distribution, see xref:#ref.webui.systems.autoinst.distribution.
...
.Procedure
. In the {webui}, upload the edited autoinstallation profile to your {productname} server.
...
. Select the proxy from the system list in SUSE Manager, click Provisioning and select the profile just created.
. Click start autoinstallation.
