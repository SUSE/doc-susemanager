...
Given the complexity of the product, the migration is an "`all-or-nothing`" procedure{mdash}if something goes wrong you will need to start all over.
...
. On the target machine install "`{productname}`" Server with the Unified Installer ({sle} 15 SP1).
+
...
When finished, you will be prompted to shut down the source machine, re-configure the network of the target machine to use the same IP address and host name as the original machine and restart it.
...
To retry setting up the new server, perform the following step on the new server machine:
...
// FIXME: 2019-05-016, ke: replace it with version 4 output
