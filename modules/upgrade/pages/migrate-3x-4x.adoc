...
[IMPORTANT]
====
Be careful when you specify the database creedentials.
...
If you have package data on external storage (for example, if you have an NFS mount at [path]``/var/spacewalk/packages``), you do not need to copy this data to the new system.
...
[IMPORTANT]
====
Make sure your external storage is mounted on the new system before starting the system for the first time.
...
The new system has the same host name and IP address as the old system.
...
Check the output of the following command (replace [literal]``<SUMA_3.2_MACHINE>`` with the actual host name of your 3.2 source system):
...
Output can lead to a corrupted transfer of the archive with the database dump. Re-visit your bash environment on the 3.2 source system (for example, the [filename]``.bashrc`` file) and make sure no extra text is printed on the shell start.


=== Retrying to Set Up the New Server

To retry setting up the new server, perform the following steps on the new server system:
