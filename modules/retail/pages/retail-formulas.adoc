...
[systemitem]``MASTER``::
overrides the Salt master autodetection; for example:
+
----
...
fallback to the local boot if the Salt master does not apply the saltboot state within this timeout (default: 60s); for example:
+
----
SALT_TIMEOUT=300
----
...
[systemitem]``DISABLE_HOSTNAME_ID`` and [systemitem]``DISABLE_UNIQUE_SUFFIX``::
Option for manipulating the names of the terminals.
...
It is used for the fallback to local boot.
