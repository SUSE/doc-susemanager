...
Your build host should be a Salt minion, running {sls}{nbsp}15 SP1.
For instructions to install Salt minion, see https://www.suse.com/documentation/suse-manager-4/4.0/suse-manager/client-configuration/manual-registration-overview.html.
...
[[retail-install-setup-11sp4terminals]]
== Terminals based on {slsa}{nbsp}11 SP4


POS Terminals based on {sls}{nbsp}11 SP{nbsp}4 can be deployed in much the same way as other terminals, with a few differences.

* You must use the {slsa}{nbsp}11 template
* {slsa}{nbsp}11 images need to be activated with the [systemitem]``SLES11 SP4 i586`` and [systemitem]``SLEPOS 11 SP4 i586`` channels
...
[[retail-install-setup-11sp3terminals]]
== Terminals based on {slsa}{nbsp}11 SP3

POS Terminals based on {sls}{nbsp}11 SP{nbsp}3 should be updated to {sls}{nbsp}11 SP{nbsp}4 as soon as possible to get support for a longer time frame.
