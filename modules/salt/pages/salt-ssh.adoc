...
If the public key doesn't exist because `salt-ssh` has not been invoked yet, a key will be generates by calling the `mgrutil.ssh_keygen` runner.

[NOTE]
====
Salt SSH generates a keypair the first time it is invoked in
