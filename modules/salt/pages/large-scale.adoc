...
In older versions, {productname} used a facility called Salt mine to check client availability.
...
Note that this will restart the `salt-minion` service on clients and generate some Salt events to be processed by the server.
...
Please do not attempt disabling any other job, as that might prevent the correct functioning of {productname}.
