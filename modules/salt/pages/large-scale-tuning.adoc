...
| Notes                | This parameter was renamed to `MacRequestWorkers`, both names are valid.
...
| Tune when            | Client count increases significantly, xref:thread-pool[`thread_pool`] increases significantly, or `SaltReqTimeoutError` or `Message timed out` errors appear in `/var/log/salt/master`.
| Value default        | 8
