...
For more information about the bind formula, see the [path]``README.rst`` file on the {productname} server: [path]``/usr/share/susemanager/formulas/metadata/bind/README.rst``.
...
In the [guimenu]``Config`` group you can set arbitrary options; click btn:[Add Item] to provide more Key/Value fields.
...
You will at least configure one zone.
