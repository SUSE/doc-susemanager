...
The size of such collection is variable; users can add or remove elements.
...
If it is `true` and the field is empty in the form, then this field will not be generated in the formula data.
The generated dictionary will miss the field name key.
If `$optional` is `false`, the formula data will contain a `<field name>: null` entry.
...
$key::
`$key` is applicable if the `edit-group` has a "shape" of a dictionary; you use it when the pillar data is supposed to be a dictionary.
