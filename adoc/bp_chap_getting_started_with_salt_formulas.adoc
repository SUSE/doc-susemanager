...
. To search for available formulas, as root execute the following command on your {susemgr} server:
+
...
----
. For more information regarding a specific formula run the following from the command line as root:
+
...
----
. To install a formula as root run:
+
...
{susemgr} requires a file called [path]``form.yml``, to describe how formula data should look within the {webui}.
[path]``form.yml`` is used by {susemgr} to generate the desired form, with values editable by a user.
...
[path]``form.yml`` contains additional information that describes how the form for a pillar should look for {susemgr}.
...
Formulas utilized with {susemgr} should follow formula conventions listed in the official documentation:
...
Formulas should be designed/created directly after a {susemgr} installation, but if you encounter any issues check the following:
...
File roots are configured on the salt-master ({susemgr}) located in:
+
...
Apply a formula to a group or system by selecting the menu:System Details[Formulas] tab of a **system's details** page or **system group**. From the menu:System Details[Formulas] page you can select any formulas you wish to apply to a group or system.
