...
[IMPORTANT]
====
If you configure both the [systemitem]``filesystem`` and [systemitem]``image`` fields in the Saltboot formula settings, your installation will fail.
Configure either the [systemitem]``filesystem`` parameter, or the [systemitem]``image`` parameter, never both.
====
