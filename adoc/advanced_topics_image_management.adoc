...
.Registry Provider Solutions``NOTE: If you require a private image registry you can use an open source solution such as Portus``.
...
.Specifying a Github or Gitlab Branch`IMPORTANT: If a branchname is not specified the master` branch will be used by default.
