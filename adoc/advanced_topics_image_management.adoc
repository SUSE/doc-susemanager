...
Next chapter covers building container images using Dockerfile. How to use Kiwi image build system see chapter <<at.images.kiwi,OS Images>>
...
OS Images are build by Kiwi image system. These can be of various types - PXE, QCOW2, LiveCD images and others.
...
.Certificate is packaged in RPM by `mgr-package-rpm-certificate-osimage` package script:
* Package script is called automatically during new {susemgr} installation.
* Upgrade scenario will (on upgrade of package `spacewalk-certs-tools`) call package script with default values.
+

However if certificate path was changed or unavailable, it is required to call package script manually with option
`--ca-cert-full-path <path_to_certificate>` after upgrade procedure is completed.
+
...
RPM with certificate is also provided in build host local repository `/var/lib/Kiwi/repo`.

====

[IMPORTANT]
.RPM with {susemgr} certificate must be specified in build source
====
...
Building of image consists of several dependent steps. When build fails, investigation of salt states results can help with identification of the source of failure.
...
* RPM with {susemgr} public certificate is up to date and available at `/usr/share/susemanager/salt/images/rhn-org-trusted-ssl-cert-osimage-1.0-1.noarch.rpm`.
+

See <<at.images.kiwi.buildhost>> on how to refresh public certificate RPM.
...
To list images available for building select menu:Images[Images]
.
