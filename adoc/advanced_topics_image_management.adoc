...
Alternatively, apply the Highstate from the {productname} Server command line:
+
----
...
Then restart the Spacewalk service:
...
The Kiwi image building feature is available for Salt minions running {sls}{mdash}12 or later.

Kiwi image configuration files and configuration scripts must be accessible in one of following ways:
...
Finally schedule and click btn:[Confirm].
. Install Kiwi and all required packages by applying the highstate.
From the system's details page select menu:States[Highstate] and click btn:[Apply Highstate].
Alternatively, apply the Highstate from the {productname} Server command line:
+
----
salt '$your_minion' state.highstate
...
. Enter a [guimenu]``Description``, a [guimenu]``Key`` name, and use the drop-down box to select the [guimenu]``Base Channel`` that should be associated with this key.
. Confirm with btn:[Create Activation Key].
...
Manage Image Profiles in the {webui}.
...
Which directory contains the image sources can be specified after the `:` character. In this example it is `OSImage/POS_Image-JeOS6`:
+
----
...
There are two ways to build an image.
In the {webui}, you can select menu:Main Menu[Images > Build], or click the build icon in the menu:Main Menu[Images > Profiles] list.

image::images_image_build.png[scaledwidth=80%]

.Procedure: Build an Image
. For this example, select menu:Main Menu[Images > Build].
. Add a different tag name if you want a version other than the default ``latest`` (Only relevant to Containers).
...
* The activation key has the correct channels associated
...
* HTTPS certificates to access the HTTP sources or Git repositories should be deployed to the minion by a custom state file or configured manually by the user.
