...
NOTE: as OS images will fill a significant amount of hard drive space, it is suggested that the OS image store (default to `/srv/www/os-images`) is in a different partition (or Btrfs subvolume) from root.
