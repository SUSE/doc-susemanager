...
OS Image building is disabled by default. You can enable it in `/etc/rhn/rhn.conf` by adding:
...
. From the menu:Main Menu[Software > Software Channels] page, enable ``SLE-Manager-Tools12-Pool`` and ``SLE-Manager-Tools12-Updates``.
Confirm with btn:[Change Subscriptions].
