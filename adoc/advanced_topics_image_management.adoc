...
and then restart Spacewalk service with `spacewalk-service restart`.
...
After image is successfuly built, inspection phase is executed where {susemgr} collects information about image:

* List of packages installed in image
* Checksum of image
* Image type and other image details

[NOTE]
====
If built image type is `PXE` type then Salt pillar is also generated. Image pillars are stored in `/srv/susemanager/pillar_data/images/` directory and allows Salt subsystem to access details about generated image.

Details include information where the pillar is located and provided, image checksums, information needed for network boot and other.

Generated pillar is available to all connected minions.
====
