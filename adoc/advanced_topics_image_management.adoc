...
The Kiwi image building feature is available for Salt minions running {sls}{nbsp}12.
Later, if you want to build {sle} 15 images, the Kiwi image build system must run on a {sle} 15 build host.
Only on a {sle} 15 build host the needed version of the kiwi software will be available.
