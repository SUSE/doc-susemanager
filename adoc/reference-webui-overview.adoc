...
* menu:Severity[] : The severity levels {info-caption} _info_ , _warning_ , and _error_ are available. For each failure a customized button (in line with the message) is available to react to that failure.
