...
* If any systems have not been checking in to {susemgr} , they are listed under menu:Overview[Inactive Systems]. Highlighting them in this way allows an administrator to quickly select those systems for troubleshooting.
+
...
Click the name of a system to see its menu:System Details[] page and apply the patch updates. Below the list is a link to menu:View All Critical Systems[] on one page.
+

.Overview > Recently Scheduled Actions
image::home_overview_critical_systems_pane.png[scaledwidth=80%]
* The menu:Overview[Recently Scheduled Actions] pane lists all actions less than thirty days old and their status: failed, completed, or pending. Click the label of any given action to view its details page. Below the list is a link to menu:View All Scheduled Actions[] on one page, which lists all actions that have not yet been carried out on your client systems.
+
