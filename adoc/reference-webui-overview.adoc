...
* menu:Notification Messages[Severity] : The severity levels image:info-circle.svg[Info,16,16], *Warning* , and *Error* are available. For each failure a customized button (in line with the message) is available to react to that failure.
