...
Entering the {susemgr} URL in a browser leads you to the [guimenu]``Sign In`` screen.
...
Login to the {susemgr} {webui} to view the menu:Main Menu[Home > Overview] page.
...
* The [guimenu]``Inactive Systems`` list provides a list of all systems that have stopped checking into {susemgr}.
+
...
* The [guimenu]``Recently Registered Systems`` pane lists all systems added to {susemgr} in the past 30 days.
...
The menu:Main Menu[Home > Notification Messages] page allows you to manage your notification messages of the {susemgr} server.
...
Set the email {susemgr} sends notifications to on the menu:Main Menu[Home > User Account > Change Email] page.
...
The menu:Main Menu[Home > User Account > Account Deactivation] page provides a means to cancel your {susemgr} service.
...
The menu:Main Menu[Home > YourPreferences] page allows you to configure {susemgr} options.
...
* [guimenu]``Time Zone`` - Set the {susemgr} interface to your local time by selecting the appropriate [guimenu]``Time Zone`` from the drop-down box.
