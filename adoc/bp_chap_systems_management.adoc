...
. Fill out the required fields. Select an menu:Activation Key[] with the menu:Push via SSH[] contact method configured. For more information about activation keys, see <<ref.webui.systems.activ_keys>>. 
...
----
. Install the [systemitem]``osad`` package on all client systems allowing communication to the osa-dispatcher on {susemgr} . The package can be found in the Tools child channel. For more information about child channels, see <<s3-sm-channel-list-child>>. 
+
