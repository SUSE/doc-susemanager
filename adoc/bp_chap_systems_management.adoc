...
On later systems, a systemd timer ([systemitem]``rhnsd.timer``) is used and controlled by the [systemitem]``rhnsd.service``.
...
You can view the status of rhnsd by typing the command [command]``service rhnsd status`` as {rootuser}.
