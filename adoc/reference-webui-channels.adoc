...
Only packages provided by a subscribed channel can be installed or updated. {susemgr} Administrators and Channel Administrators have channel management authority.
...
Click it to grant specific users subscription permissions to a channel. {susemgr} administrators and channel administrators can always subscribe any channels to a system.
...
For more information on user management and the [guimenu]``User Details`` page, see ifndef::env-github,backend-html5[]
<<ref.webui.users>>.
endif::[]
ifdef::env-github,backend-html5[]
