...
The bootstrap script you create is executed on the traditional client when it is registered, and ensures all the neccessary packages are deployed to the client.
There are some important parameters contained in the bootstrap script which ensure the client system can be correctly assigned to its base channel, including activation keys, and GPG keys.

Additionally, it is important that you check the repository information carefully, to ensure it matches the base channel repository.
...
The second method uses the bootstrap script, and is mostly similar to the procedure described in <<registering.clients.traditional>>{mdash}the difference is enabling [guimenu]``Bootstrap using Salt`` and the activation key option [guimenu]``Configuration File Deployment`` that applies highstate automatically.
...
The rest of this section assumes you have created a SUSE Manager tools repository.
