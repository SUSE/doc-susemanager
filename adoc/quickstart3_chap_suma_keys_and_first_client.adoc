...
When you create your bootstrap script, add the public key fingerprint with the [systemitem]``ORG_GPG_KEY`` parameter.
====
...
You can use a bootstrap repository, a bootstrap script, or install using the the {webui}.
...
You will also need to have fully synchronized a base channel from the {webui} for clients to obtain software packages (for example: `SLES12-SP4-Pool_for_x86_64`).
