...
. Open the [guimenu]``Activation Key Details`` page by clicking the btn:[Create Key] button in the upper right corner.
+
...
. When the base channel is selected, the list of available child channels will fetched and displayed in real time.
Select the child channels you need (for example, the {susemgr} Tools child channel).
