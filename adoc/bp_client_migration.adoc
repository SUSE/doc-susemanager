...
To upgrade the SP version on SLE{nbsp}12 (for example, upgrading from SLE{nbsp}12, or SLE{nbsp}12{nbsp}SP1 or SP2, to SLE{nbsp}12{nbsp}SP3 or later) can be fully automated and requires no additional preparation.
...
In the case of SLE{nbsp}12, this is currently SP3 (August 2018).
====
...
In order to switch from the old SLE{nbsp}12{nbsp}SP3 base channel to the new SLE{nbsp}15 channel, you need an activation key.
...
== Migrating SLE{nbsp}12 or later to version 12{nbsp}SP3


Existing SLE{nbsp}12 clients (SLE) may be upgraded to SP3 with the menu:SP Migration[] procedure provided by the {webui}.
...
For the upgrade to SLE{nbsp}12{nbsp}SP3, at least the [systemitem]``SLES12-SP3-Pool``
...
.Procedure: Migrating SLE{nbsp}12 Client to SP3
...
. From this tab you will see the installed products listed on your client. Select the wanted menu:Target Products[] (if there is more than one), which is [systemitem]``SUSE Linux Enterprise Server 12{nbsp}SP3`` .
+
