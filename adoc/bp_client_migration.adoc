...
. On the system's detail page (<<fig-sles-old-details>>) select the menu:Software[] tab, then the menu:SP Migration[] tab (<<fig-sles-old-spmigr>>).
+
...
. On <<fig-sles-migration-channels>>, menu:Schedule Migration[] , and on <<fig-sles-migration-schedule>>, menu:Confirm[] .
+
