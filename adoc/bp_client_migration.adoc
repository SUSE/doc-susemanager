...
In the case of SLE 12, this is currently SP3 (August 2018).
====
...
** Enter a [guimenu]``Distribution Label`` for your distribution (for example, use ``autumn2018``) as a label and specify the [guimenu]``Tree Path``, which is the root directory of the {sls} 15 installation sources (for example, in this case [path]``/mnt/sle15``). As the [guimenu]``Base Channel`` use the update target distribution [systemitem]``SLE-Product-SLES15-Pool for x86_64``.
...
** Decide about [guimenu]``Add-On System Types``.  If in doubt, see https://www.suse.com/documentation/sles-15/book_quickstarts/data/art_modules.html ({slea} Modules & Extensions Quick Start).
...
** Enter in the text field `registration_key=` and the key from the preparation above.
