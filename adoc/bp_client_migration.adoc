...
.Client System Details Page [[fig-sles-old-details]]
...
. On the system's detail page (<<fig-sles-old-details>>) select the menu:Software[] tab, then the menu:SP Migration[] tab (<<_fig_sles_old_spmigr>>). 
+

.SP Migration Page [[fig-sles-old-spmigr]]

image::sles_old_details_spmigration.png[scaledwidth=80%]
. On <<fig-sles-old-spmigr>> you will see the installed products listed on your client. Select the wanted menu:Target Products[] (if there is more than one), which is [systemitem]``SUSE Linux Enterprise Server 12 SP3`` . 
+

.SP Migration Target [[fig-sles-migration-target]]
...
.SP Migration Channels [[fig-sles-migration-channels]]

image::sles_migration_channels.png[scaledwidth=80%]
. On <<fig-sles-migration-channels>>, menu:Schedule Migration[] , and on <<_fig_sles_migration_schedule>>, menu:Confirm[] . 
+

.Schedule Client System Migration [[fig-sles-migration-schedule]]
...
.Migrated Client System [[fig-sles-migrated]]
...
. Go to the system via menu:Systems[] and click the name of the system. Then on the systems details page, click menu:Provisioning[Autoinstallation > Schedule] , and choose the {ay} XML profile you have uploaded in <<bp.proc.client.migration.sle11.prep>>, <<_st.maintenance_migration.upload>>. 
