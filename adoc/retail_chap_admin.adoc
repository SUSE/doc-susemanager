...
Install the [package]``python-susemanager-retail`` package on the central server.

.Procedure: Configuring multiple Branch Servers

. Create a YAML file describing the infrastructure you intend to install.
An example YAML file is at the end of this section, for you to reference.
...
.Example: YAML mass Branch Server configuration file
----
...
=== Synchronize OS images to the branch servers

Synchronization of images is managed by salt state [systemitem]``image-sync``.

.Procedure: Synchronize OS images to branch server
...
////
I'm not sure why this is here, or what it attaches to. -LKB
