...
** Increase this parameter further if file management states fail with the error "Unable to manage file: Message timed out". As Salt master worker threads tend to consume significant RAM, keep usage monitored when increasing this value in relatively small increments (eg. 20) until failures are not produced any longer.
