...
Before explaining the various automated installation options on this page, the next two sections provide an introduction to {ay}
 (<<ref.webui.systems.autoinst.ay_intro>>) and Kickstart (<<ref.webui.systems.autoinst.kick_intro>>).
...
Starting with <<ref.webui.systems.autoinst.profiles>>, {ay}
...
<<s3-system-ks-software>> shows the subtabs that are available from the menu:Software[]
 tab.
