...
----
+
This example directs Salt to look at the [filename]``pkg_download_endpoints.sls`` file to determine the base URL to use.
You can adjust this file to target different minions or groups, depending on your environment.
+
. Remain in [path]``/srv/pillar/`` and create a file called [filename]``pkg_download_endpoints.sls`` with the base URLs you want to use.
For example:
+
----
