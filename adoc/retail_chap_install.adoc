...
=== Procedure Create required system groups

{smr} uses two generic system groups for administration purposes.
Theses system groups needs to be manually created before configuration of Retail system.

Check and create mandatory system groups:
...
Branch server can be configured manually using Formulas or automatically using [command]``retail_branch_init`` command on {susemgr} server.

. Branch server configuration performed using the [command]``retail_branch_init`` command:
+
...
For manual configuration or fine-tuning using Formulas see xref:retail_salt_formulas.adoc[Retail Formulas documentation]

[IMPORTANT]
====
In case of manual configuration of the branch server using formulas, mandatory branch group must be manually created.
This branch group has a name of the Branch Server ID used in configuration of PXE formula.
====
