...
{smr} can be installed in various ways, recommended installations require following steps:
...
Available SUSE channels are essential for functional image building as well as day-to-day operations. Completely download the
required channels, for instructions see the xref:command-line-tools.adoc[Syncing {susemgr} Repositories from SCC (mgr-sync)].

Here is a channel overview required for {smr} functionality:
----
...
After all required SUSE channels are synchronized, channels with custom software should be created. These channels will be then
used to provide custom software for OS image building. For instructions see the xref:reference-webui-channels.adoc[Software Channels Reference]
...
See xref:quickstart3_chap_suma_keys_and_first_client.adoc[Registering Clients] for instructions to install Salt minions and register them to {susemgr}.

[WARNING]
====
Do not install Build Host as a traditionally managed client.
====
...
For instructions to install Salt-based proxy minions and register them to {susemgr}, see xref:advanced_topics_proxy_quickstart.adoc#at.manager.proxy.inst[{susemgr} Advanced
Topics].
...
[WARNING]
====
Do not install Branch Server as a traditionally managed client.
====
