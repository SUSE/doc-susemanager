...
* Sync All Orgs to Slave? Checking this checkbox will synchronize all organizations to the slave {productname}.
...
When the master and slave servers are configured, start the synchronization on the slave server by executing [command]``mgr-inter-sync``:
