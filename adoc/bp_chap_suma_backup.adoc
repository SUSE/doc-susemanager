...
Backing up only these files and directories requires reinstalling the {productname} RPMs and re-registering {productname}
.
...
Another method is to back up all the files and directories mentioned above but reinstall the {productname} without re-registering it.
...
[NOTE]
====
Regardless of the backup method used, when restoring {productname} from a backup, you must run the following command to schedule the recreation of search indexes the next time the [command]``rhn-search`` service is started:
...
. The default storage location for {productname} is: [path]``/var/lib/pgsql/`` . You would like to move it, for example to: [path]``/storage/postgres/`` . To begin, stop the running database with:
+
