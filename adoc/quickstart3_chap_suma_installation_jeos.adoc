...
. In previous releases, you had to check [path]``/etc/fstab`` for correctness as follows (_updated tools shipped with recent SPs will no longer require human intervention._):
A mount point was created automatically for [path]``/var/lib/pgsql/`` (even when not installed).
You had to remove or comment out from the [path]``/etc/fstab`` entries.
As root, you had to edit [path]``/etc/fstab`` and comment out or remove the line:
+
...
====
If you did not remove this line from fstab the first time you shut down the server you would lose your database.
This occured because of duplicated entries in the fstab file.
====
+
