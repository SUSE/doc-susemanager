...
interface, refer to <<ref.webui.systems.autoinst.distribution>>. 
...
interface, refer to <<ref.webui.systems.autoinst.profiles>>. 
...
interface, refer to <<s4-sm-system-details-kick>>. 
...
The [option]``--profile=``[replaceable]``string`` specifies the name of one of the profiles added in <<advanced.topics.cobbler.addprofile>>. 
