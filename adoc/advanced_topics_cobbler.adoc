...
{productname} features the Cobbler server that allows administrators to centralize their system installation and provisioning infrastructure.
...
* For system installation with PXE, you must have a TFTP server installed and configured. By default, {productname} installs such a TFTP server.
...
.Supported Languages
====
If {productname} complains about language '``en``' was not found within the list of supported languages available at [path]``/usr/share/YaST2/data/languages``, remove the lang in [path]``/etc/cobbler/settings`` or add a proper value such as ``en_US``.
...
{productname} uses the [daemon]``atftpd`` daemon, but it can also operate with Xinetd/TFTP.
...
It is possible to sync cobbler generated TFTP contents to {productname} Proxies to perform PXE booting via these proxies.



==== Installation

On the {productname} Server as root, install the package [systemitem]``susemanager-tftpsync``:
...
As root execute [path]``configure-tftpsync.sh`` on {productname} Server:
...
If content, start the {productname} server with the following command:
...
The {productname} Web interface facilitates creating variables for use with Kickstart distributions and profiles.
To create a Kickstart profile variable, refer to <<s4-sm-system-kick-details-variables>>.

Kickstart variables are part of an infrastructural change in {productname} to support templating in Kickstart files.
