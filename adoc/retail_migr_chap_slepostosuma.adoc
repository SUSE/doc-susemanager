...
Branch servers must be based on SUSE Manager Proxy systems; for more information, see xref:retail_chap_install.adoc#retail.sect.install.branch[Installing and Registrering].
...
For instructions to install Salt-based proxy minions and register them to SUSE Manager, see SUSE Manager Advanced Topics.
...
In this procedure, you migrate the SLEPOS infrastructure and branches one by one, exporting and then importing.
...
If you want to preserve any global changes in your {sme} settings, remove the [systemitem]``global`` hardware types from the resulting [path]``retail.yml`` file before importing it.
Alternatively, you can import [path]``retail.yml`` using this command to only import the new information contained in the file:
**TODO**: What does 'new' mean here?

----
retail_yaml --only-new option.
----
