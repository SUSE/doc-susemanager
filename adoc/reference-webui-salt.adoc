...
Use `Remote Commands` to execute remote commands on your Salt minions.
You can also define a `State Catalog` for creating a collection of salt system states.
...
[WARNING]
====
All commands run from the `Remote Commands` page are executed as {rootuser} on minions.
