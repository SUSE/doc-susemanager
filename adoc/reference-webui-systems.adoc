...
If you click menu:Main Menu[Systems > Overview], an overview of all Systems appears.
Here you can select systems to perform actions on and create system profiles.
...
The menu:System Overview[] page provides a summary of your systems, including their status, number of associated patches (errata) and packages, and their so-called system type.
...
Clicking menu:View System Groups[] at the top of the menu:Overview[] page takes you to a similar summary of your system groups.
It identifies group status and displays the number of systems contained.
Then, clicking the number of systems takes you to the menu:Systems[] tab of the menu:System Group Details[] page, while clicking the group name takes you to the menu:Details[] tab for that system group.
For more information, see:
// TODO update link on github
 <<_ref.webui.systems.systemgroups.details>>

You can also click menu:Use in SSM[] in the menu:System Groups[] section to go directly to the menu:System Set Manager[].
For more information, see:
//TODO update link on github
...
* Select box: Systems without a system type cannot be selected. To select systems, mark the appropriate check boxes.
Selected systems are added to the menu:System Set Manager[], where actions can be carried out simultaneously on all systems in the set.
 For more information, see:
// TODO Update github link
 <<_ref.webui.systems.ssm>>.
* menu:System[] : The name of the system specified during registration.
The default name is the host name of the system.
Clicking the name of a system displays its menu:System Details[] page.
For more information, see:
// TODO Update github link
<<_s3_sm_system_details>>
+
...
* menu:Updates[] : Shows which type of update action is applicable to the system or confirms that the system is up-to-date.
...
* menu:Packages[] : Total number of package updates for the system, including packages related to patch alerts and newer versions of packages not related to patch alerts.
...
* menu:Base Channel[] : The primary channel for the system based on its operating system.
For more information, see:
// TODO update github link
...
The menu:All[] page contains the default set of your systems.
...
Clicking the host name of such a guest system displays its menu:System Details[] page.
...
The menu:Out of Date[] page displays all systems where applicable patch alerts have not been applied.
...
menu:Packages[] shows how many installed packages are not available in the channels assigned to the system.
...
The menu:Without System Type[] page displays systems without a System Type.
System types are:
...
The menu:Ungrouped[] page displays systems not yet assigned to a specific system group.
...
The menu:Inactive Systems[] page displays systems that have not checked in with {susemgr} for 24 hours or more.
...
Refer to <<_bp.contact.methods.rhnsd>> for instructions on restarting and troubleshooting.
...
The menu:Recently Registered[] page displays any systems that have been registered in a given period.
...
The menu:Proxy[] page displays the {susemgrproxy} Server systems registered with your {susemgr} server.
...
The menu:Duplicate Systems[] page lists current systems and any active and inactive entitlements associated with them.

image::systems_duplicate_systems.png[scaledwidth=80%]

Active entitlements are in gray, while inactive entitlements are highlighted in yellow and their check boxes checked by default for you to delete them as needed by clicking the menu:Delete Selected[] button.
Entitlements are inactive if the system has not checked in with {susemgr} in a time specified via the drop-down box menu:A system profile is inactive if its system has not checked in for[].

You can *filter* duplicate entitlements by:
...
Then click the respective tab.
You may filter further by inactive time or typing the system's host name, IP address, IPv6 address, or MAC address in the corresponding menu:Filter by[] text box.

To compare up to three duplicate entitlements at one time, click the menu:Compare Systems[] link in the menu:Last Checked In[] column.
Inactive components of the systems are highlighted in yellow.
You can then determine which systems are inactive or duplicate and delete them by clicking the menu:Delete System Profile[] button.
Click the menu:Confirm Deletion[] button to confirm your choice.
...
The weighting is defined via the menu:System Details[] page.
...
To change system types, select the systems you want to modify, and choose either menu:Add System Type[] or menu:Remove System Type[]
...
Here and on any other page, clicking the name takes you to the menu:System Details[] page of the client, where all kinds of administrative tasks can be performed, including the removal of a system.
...
By default, the menu:Details[Overview] subtab is displayed.
...
[NOTE]
====
If updates are available for any entitled system, the message menu:Software Updates Available[] appears, displaying the number of critical and non-critical updates and the sum of affected packages.
To apply these updates, click menu:Packages[] and select some or all packages to update, then click menu:Upgrade Packages[].
====
+
...
To prevent the application of automated patch updates, deselect menu:Auto Patch Update[] from the menu:System Details[Details > Properties] subtab.
...
Clicking the menu:Edit These Properties[] link right beside the menu:System Properties[] title opens the menu:System Details[Properties] subtab.
On this page, edit any text you choose, then click the menu:Update Properties[] button to confirm.
...
Contact Method:::
Select between menu:Pull[], menu:Push via SSH[] and menu:Push via SSH tunnel[].
...
To confirm any changes to these fields, click the menu:Update Properties[] button.
...
Select a date and time to execute the command, then click menu:Schedule[] or add the remote command to an action chain.
...
Unlike typical activation keys, which are not associated with a specific system ID, keys created here do not show up within the menu:Activation Keys[] page.
...
If the hardware profile looks incomplete or outdated, click the menu:Schedule Hardware Refresh[] button.
...
Select an menu:Organization Name[] and click menu:Migrate System[] to initiate the migration.
...
To add a new note, click the menu:Create Note[] link, type a subject and write your note, then click the menu:Create[] button.
To modify a note, click its subject in the list of notes, make your changes, and click the menu:Update[] button.
To remove a note, click its subject in the list of notes then click the menu:delete note[] link.
...
Unlike menu:Notes[], menu:Custom Info[] is structured, formalized, and can be searched.
Before you can provide custom information about a system, you must have menu:Custom Information Keys[].
To create such keys, click menu:Custom System Info[] in the left bar.
For more information, see <<_ref.webui.systems.cust_info>>.

image::system_details_traditional_custom_info.png[scaledwidth=80%]

When you have created one or more keys, you may assign values for this system by selecting the menu:Create Value[] link.
Click the name of the key in the resulting list and enter a value for it in the menu:Description[] field, then click the menu:Update Key[] button.
...
To apply updates, select them and click the menu:Apply Patches[] button.
Double-check the updates to be applied on the confirmation page, then click the menu:Confirm[] button.

The action is added to the menu:Pending Actions[] list under menu:Schedule[].
Patches that have been scheduled cannot be selected for update.
Instead of a check box there is a clock icon.
Click the clock to see the menu:Action Details[] page.

A menu:Status[] column in the patches table shows whether an update has been scheduled.
...
Clicking a status other than `None` takes you to the menu:Action Details[] page.
This column corresponds to the one on the menu:Affected Systems[] tab of the menu:Patch Details[] page.
...
Packages::
The default display of the menu:Packages[] tab describes the options available and provides the means to update your package list.
To update or complete a potentially outdated list, possibly because of the manual installation of packages, click the menu:Update Package List[] button in the bottom right-hand corner of this page.
...
Search for the wanted packages by typing its name in the menu:Filter by Package Name[] text box, or by clicking the letter or number corresponding to the first character of the package name.
Click a package name to view its menu:Package Details[] page.
To delete packages from the system, select their check boxes and click the menu:Remove Packages[] button on the bottom right-hand corner of the page.
A confirmation page appears with the packages listed.
Click the menu:Confirm[] button to remove the packages.

Upgrade::
Displays a list of packages with newer versions available in the subscribed channels.
click the latest package name to view its menu:Package Details[] page.
To upgrade packages immediately, select them and click the menu:Upgrade Packages[] button.
Any EULAs will be accepted automatically.

Install::
Install new packages on the system from the available channels.
Click the package name to view its menu:Package Details[] page.
To install packages, select them and click the menu:Install Selected Packages[] button.
...
If locking should happen later, select the date and time above the menu:Request Lock[] button, then click it.
A small lock icon marks locked packages.
To unlock, select the package and click menu:Request Unlock[], optionally specifying the date and time for unlocking to take effect.
+
...
Select a stored profile from the drop-down box and click the menu:Compare[] button.
To compare with packages installed on a different system, select the system from the associated drop-down box and click the menu:Compare[] button.
To create a stored profile based on the existing system, click the menu:Create System Profile[] button, enter any additional information you want, and click the menu:Create Profile[] button.
These profiles are kept within the menu:Stored Profiles[] page linked from the left bar.
+
...
Then click the menu:Sync Packages to[] button.
On the confirmation screen, review the changes, select a time frame for the action, and click the menu:Schedule Sync[] button.
+
...
Click the chain icon right to a channel name to view its menu:Channel Details[] page.
...
If you enable menu:include recommended[], recommended child channels are automatically selected for subscription.
Starting with {sle} 15, child channels can depend on each other.
...
In the menu:Configuration Overview[], click the menu:Add[] links to add files, directories, or symbolic links.
...
Determine which channel provides which file by examining the menu:Provided By[] column below.
Some of these centrally-managed files may be overridden by locally-managed files.
Check the menu:Overridden By[] column to find out if any files are overridden, or click menu:Override this file[] to provide such an overriding file.
...
Use menu:Copy Latest to System Channel[] to install a configuration on the local system only.
When done, the file will end up on the menu:Locally-Managed Files[] subtab.
...
After the diff has been performed, go to menu:Recent Events[] in <<_s5_sdc_configuration_overview>> to see the results.
