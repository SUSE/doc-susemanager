...
.Package Conflict
====
If {productname} identifies package updates for the system, but the package manager (such as zypper or {yast}) responds with a message such as "Your system is fully updated", a conflict likely exists in the system's package profile or in the configuration file of the package manager.
...
==== menu:System Details[Configuration > Add Files] [Management]
