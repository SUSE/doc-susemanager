...
For example, associate the key with packages (for example, the [package]#rhncfg-actions#  package) and groups.
...
When the activation key is created, you can add packages with selecting the key (menu:Main Menu[Systems > Activation Keys]), then on the activation key details page, go for the [guimenu]``Packages`` tab and add [package]#osad#.
...
VHMs can manage one or more virtual hosts, and each virtual host can can contain one or more virtual guests.
...
After it has been created and configured, VHMs can be started or refreshed manually through the {webui}, by navigating to menu:Systems[Virtual Host Managers], selecting the VHM you wish to start, and clicking btn:[Refresh Data]
...
In order to use the [command]``virtual-host-gatherer`` tool, you will need configure your VHMs using XMLRPC APIs, after creating them in the {webui}.
