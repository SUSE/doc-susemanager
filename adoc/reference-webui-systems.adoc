...
Then the systems will get subscribed to the required {susemgr} tools channel and required [package]#rhncfg*# packages will get installed.
...
If you use [guimenu]``Autoinstall by IP Address``, {susemgr} will automatically pick a profile for each system so that the system's IP address will be in one of the IP ranges specified in the profile itself.
If such a profile cannot be found, {susemgr} will look for an organization default profile and apply that instead.
...
If any of the systems connect to {susemgr} via a proxy server, choose either the [guimenu]``Preserve Existing Configuration`` radio button or the [guimenu]``Use Proxy`` radio button.
