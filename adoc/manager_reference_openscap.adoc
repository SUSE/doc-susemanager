...
NOTE: If [command]``rhnsd`` or [command]``osad`` are running on the client system, the action will be picked up by these services.
On systemd-based systems, you can check if they are running with:
+
----
service rhnsd start
service osad start
