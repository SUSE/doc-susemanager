...
{susemgr} cannot distinguish between different instances that use the same system ID.
If you register a second instance with the same system ID as a previous instance, {susemgr} will overwrite the original system data with the new system data.
...
Once you have deleted the minion ID file, re-run the bootstrap script, and restart the minion to see the cloned system in {susemgr} with the new ID.
