...
This Getting Started Guide provides guidance on setting up {susemgr} server with KVM.
...
{susemgr} is a solution for organizations that require absolute control over maintenance and package deployment on their servers.
...
{susemgr} allows you to maintain a high level of security while effectively managing system life-cycle requirements.

{susemgr} uses _Salt_ to provide event-driven configuration and management control.
The Salt-master orchestrates thousands of Salt-minions ({susemgr} Clients) using remote execution.
...
{susemgr} can be integrated with your network infrastructure in multiple ways.
This book will guide you through an initial proof-of-concept setup, using these steps:

. Install an operating system (either JeOS or {slsa}) for use with {susemgr}
. Install {susemgr} Server
. Register {susemgr} with {scc}
. Perform initial setup of your {susemgr} Server
...
You will need to create an account with {scc} before you install {sls} and {susemgr}.
...
This section details the networking and port requirements for {susemgr}.
...
In a production environment, {susemgr} server and its clients should always use a firewall.
