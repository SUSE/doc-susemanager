...
Themenu:Main Menu[Audit > CVE Audit] page will display a list of client systems with their patch status regarding a given CVE (Common Vulnerabilities and Exposures) number.
...
* The image:patch-status-affected-patch-applicable.png[[orange],scaledwidth=1em] state is displayed when {susemgr} has at least one patch in an assigned channel. This might mean that, after installing such a patch, others might be needed—users should double check the CVE Audit page after applying a patch to be sure that their systems are not affected anymore. 
