...
=== Prepare the Shared iSCSI Storage Node
...
.Procedure: Post Installation Setup of the iSCSI Storage Server
...
<1> In the *scp* snippet *node2* is the user, and we added the servers short host name to the `/etc/hosts` file on node1 with its IP address:
+
./etc/hosts on node1
----
192.168.188.2 node2
