...
This document describes {suse} recommended best practices for {susemgr}.
This information has been collected from a large number of successful {susemgr} real world implementations and includes feedback provided by product management, sales and engineering.
...
<<common_version_info.adoc#common-version-info, Version Information>>
endif::[]
...
During initial setup {susemgr} will request a product [systemitem]``Registration Key``.
...
On the {susemgr} product page click btn:[TRY SUSE MANAGER].
...
.Users and Passwords
During both the {sle} installation and setup of {susemgr} several users and passwords will be created:
...
{susemgr} and {susemgrproxy} both contact several external addresses in order to maintain updates and subscriptions.
...
For {susemgr} to function properly it requires the following pre-configured components within your network.
...
As {susemgr} will likely be managing a large number of systems (quite possibly numbering in hundreds or even thousands), networking hardware that increases bandwidth becomes increasingly more valuable.
...
In the case of {susemgr} the DNS server must be resolvable both via DNS and reverse lookup.
...
====
Microsoft NT Lan Manager can be configured for use with basic authentication and will work with {susemgr} but authentication using native (NTLM) Microsoft protocols is not supported.
====

.Open Port List
During the setup process of {susemgr} all required ports will be opened automatically.
...
| 22   | TCP      | Required for SSH connections.
| 67   | TCP/UDP  | Required for DHCP connections
| 69   | TCP/UDP  | Required for TFTP connections, used to support PXE services
| 80   | TCP      | Required for HTTP connections, used to support WebGUI, client, and proxy server traffic
| 123  | UDP      | Required for NTP time service connections
| 443  | TCP      | Required for HTTPS connections, used to support WebGUI, client, and proxy server traffic
| 5222 | TCP      | Required for XMPP client connections, used for communications with the [systemitem]``osad`` daemon on client systems
| 5269 | TCP      | Required for XMPP server connections, used for pushing actions to {susemgrproxy}
| 4505 | TCP      | Required for Salt connections, used by the Salt-master to accept communication requests from minions
| 4506 | TCP      | Required for Salt connections, used by the Salt-master to accept communication requests from minions
|===
...
====
When your network requires denying external network access to and from {susemgr}, an RMT or SMT Server may be registered against {susemgr}.
...
.Disk Space
{susemgr} stores information in several directories.
...
.Supported Databases
{susemgr} 3 and later no longer provides support for an external Oracle database.
The default database is an embedded PostgreSQL.
During {susemgr} setup the database will be created and configured.
