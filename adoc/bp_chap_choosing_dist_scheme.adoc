...
.Write Permissions for RMT User`IMPORTANT: The directory being written to must be writeable for the same user as the rmt service.
...
.Keeping the Disconnected Server Up-to-date``NOTE: [command]smt-sync`` also exports your subscription data.
To keep {susemgr}
