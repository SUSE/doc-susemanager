...
* Live patching
* If you use `server.susemanager.fromdir = ...` option in rhn.conf of SUSE Manager, Ubuntu Client Tools will not work.
  (reason: SMT cannot mirror Debian repositories (yet) and so cannot create it in that directory the correct files. We are waiting for SMT to release the feature/fix to mirror Debian repositories). 
