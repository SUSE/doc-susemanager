...
. On the client, open the [filename]``/etc/apt/sources.list.d/suma_client_tools.list`` file in your preferred text editor, and add this line to the bottom of the file:
+
----
...
. In the [guimenu]``Create Repository`` window, use these values to create a new repository:
+
...
. In the [guimenu]``Create Software Channel`` window, create the channel as required for your environment.
